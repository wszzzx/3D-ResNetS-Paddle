...
def generate_model(opt):
    ...
    if opt.model == 'resnet':
        model = resnet_3d.generate_model(model_depth=opt.model_depth,            # 50
...
def load_pretrained_model(model, pretrain_path, model_name, n_finetune_classes):
    if pretrain_path:
        ...
        # yyz s
        pretrain = paddle.fluid.io.load_program_state(pretrain_path)
        model.set_dict(pretrain)
        # yyz e
        ...
        # yyz s
        tmp_model.fc = fluid.dygraph.Linear(tmp_model.fc.in_features,n_finetune_classes)
        # yyz e
